...
const formatTime = (time, type = '3') => {
  ...
  switch (type) {
    ...
    case '1':
      time = new Date(time)
      str = `${time.getFullYear()}-${padStart2(
        time.getMonth() + 1
      )}-${padStart2(time.getDate())} ${padStart2(time.getHours())}:${padStart2(
        time.getMinutes()
...
function timeType3(t) {
  ...
  if (nowTime - time < 60 * 1000) {
    ...
    return `${padStart2(new Date(t).getMonth() + 1)}月${padStart2(
      new Date(t).getDate()
    )}日`
  } else {
    return `${new Date(t).getFullYear()}-${padStart2(
      new Date(t).getMonth() + 1
    )}-${padStart2(new Date(t).getDate())}`
  }
}
