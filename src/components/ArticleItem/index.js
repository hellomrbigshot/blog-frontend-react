    ...
    return (
        <div className='mb-24'>
            ...
            <div className='text-xs mb-3 text-gray-500'>
                ...
                <span>
                    {' '}
                    | 作者<Link to={`/user/info/${article.get('create_user')}`} className='ml-1 border-b border-solid text-gray-600 hover:text-gray-700 hover:border-gray-700'>{article.get('create_user')}</Link>
                </span>
...
function showTags(tags) {
    ...
    return tags.map((tag, i) => {
        return (
          <Fragment key={i}>
            <Link className='ml-1 border-b border-solid text-gray-600 hover:text-gray-700 hover:border-gray-700' to={`/tag/detail/${tag}`}>{tag}</Link>
            {
