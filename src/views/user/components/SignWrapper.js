    ...
    <div className='fixed top-0 left-0 right-0 bottom-0 overflow-hidden  bg-white flex items-center justify-center'>
      ...
      <div className='p-12 rounded-md shadow w-96'>
        <div className='text-center text-xl font-medium text-gray-500 flex items-center justify-center'>
          <Link className={classnames('p-2 border-b-4 border-solid hover:text-blue-500', { 'border-blue-500 text-blue-500': type === 'SIGNIN', 'border-white': type !== 'SIGNIN' })} to={signinPath}>
            登录
          </Link>
          <b className='p-2 border-b-4 border-solid border-white'>·</b>
          <Link className={classnames('p-2 border-b-4 border-solid hover:text-blue-500', { 'border-blue-500 text-blue-500': type === 'SIGNUP', 'border-white': type !== 'SIGNUP' })} to={signupPath}>注册</Link>
        </div>
        { children }
      </div>
    </div>
